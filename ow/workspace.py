...
def make_odoorc(ws: WorkspaceConfig, config: Config) -> str:
    ...
    for alias in ws.repos:
        ...
        if is_odoo_main_repo(repo_dir):
            ...
            addons.extend(str(p) for p in find_addon_paths(repo_dir))
...
def make_odools_toml(ws: WorkspaceConfig, ws_dir: Path) -> str:
    main_repo_alias: str | None = None
    path_items: list[str] = []
    main_path_items: list[str] = []

    for alias in ws.repos:
        repo_dir = ws_dir / alias
        if is_odoo_main_repo(repo_dir):
            main_repo_alias = alias
            main_path_items = [
                f'    "./{alias}/addons"',
                f'    "./{alias}/odoo/addons"',
            ]
            ...
            for p in find_addon_paths(repo_dir):
                ...
                path_items.append(f'    "./{rel}"')

    path_items += main_path_items
    paths_str = ",\n".join(path_items)
    odoo_path = f"./{main_repo_alias}" if main_repo_alias else "./community"
...
def cmd_apply(config: Config, name: str | None = None) -> None:
    ...
    for ws in workspaces:
        ...
        (ws_dir / "odools.toml").write_text(make_odools_toml(ws, ws_dir))
